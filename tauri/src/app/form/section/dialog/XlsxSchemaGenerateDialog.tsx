...
import { XlsxSchemaEditDialog } from "./XlsxSchemaDialog";

const formId = "xlsxSchemaGenerateForm";

// generate/refreshにgenerateType=xlsxSchemaのオプション構造を要求する
function withGenerateType(values: FormValues): FormValues {
	return { ...values, "-generateType": "xlsxSchema" };
}

function XlsxSchemaGenerateDialog({
	handleSave,
	handleDialogClose,
}: {
	...
	const [generated, setGenerated] = useState<XlsxSchema | null>(null);
	...
	const handleExecute = async () => {
		...
		setExecuteError("");
		try {
			const schema = await postDialogForm<XlsxSchemaBuilder>(
				environment.apiUrl,
				"xlsx-schema/generate",
				withGenerateType(values),
			);
			if (!abortControllerRef.current?.signal.aborted) {
				setGenerated(XlsxSchema.build(schema));
			}
		} catch (ex) {
			if (!abortControllerRef.current?.signal.aborted) {
	...
	return createPortal(
		<FullDialog onClose={handleDialogClose}>
			<div className="overflow-y-auto max-h-[80vh] min-w-[600px] p-4">
				<DialogTitle>Generate Xlsx Schema</DialogTitle>
...
export function XlsxSchemaGenerateButton({ setPath }: ResourceEditButtonProp) {
	const renderDialog = (open: boolean, closeDialog: () => void) => {
		if (!open) {
			return null;
		}
		return (
			<XlsxSchemaGenerateDialog
