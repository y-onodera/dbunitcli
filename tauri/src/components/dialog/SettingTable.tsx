...
export default function SettingTable<T>({
	...
	updateSettings = (current, before, after) =>
		current.map((s) => (s === before ? after : s)),
				...
				<tbody>
						...
						<tr key={getKey(setting)}>
							...
							<td className="px-6 border">
								<div className="flex">
									<EditButton
										handleClick={() =>
											setSelectSettings({ setting, action: "update" })
										}
									/>
									<DeleteButton
										handleClick={() =>
											setSettings((current) => deleteSettings(current, setting))
										}
									/>
									<CopyButton
										handleClick={() =>
											setSelectSettings({ setting, action: "add" })
										}
									/>
								</div>
							</td>
					...
					<tr>
						<th scope="row" className="px-6 py-4">
							<AddButton
								handleClick={() =>
									setSelectSettings({ setting: newSetting(), action: "add" })
								}
							/>
						</th>
