...
type MenuEditProp = {
    name:string;
    handleMenuDelete:Function;
    handleMenuRename:Function;
    handleMenuCopy:Function;
}
export default function NameEditMenu() {
    ...
    return (
      <>
        { editName.name &&
            <div ref={wrapperRef} className="absolute z-10 p-4 pb-0 text-gray-900 bg-white border border-gray-100 rounded-lg shadow-md"
                 style={editName.afterEdge ? {left:editName.x, top:editName.y - 150} : {left:editName.x, top:editName.y}}>
              <MenuEdit
                 name={editName.name}
                 handleMenuDelete={()=> {
                   handleMenuDelete();
...
function MenuEdit(prop:MenuEditProp) {
    ...
    return (
          <ul className="space-y-4">
              <li>
                <button onClick={()=>prop.handleMenuDelete()} 
              ...
              <li>
                <div className="flex" >
                  ...
                  <input type="text" onChange={(text)=>handleRenameTextOnChange(text.target.value)} className="p-2.5 
                      w-50 
                      z-20 
                      ...
                      value={newName} hidden={!visible}/>
                      <button className="p-1" hidden={!visible} onClick={()=>{setVisible(!visible);prop.handleMenuRename(newName);}}>
              ...
              <li>
                <button onClick={()=>prop.handleMenuCopy()} 
                  className="flex p-1 items-center text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-500 group">
                  <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24">
                    <path d="M0 0h24v24H0z" fill="none"/>
