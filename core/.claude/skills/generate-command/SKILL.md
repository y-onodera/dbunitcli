...
1. **データロード**: 実データ行が要るか、スキーマ（列定義・型）だけで足りるか。`GenerateType`の`loadData()`/`useJdbcMetaData()`をオーバーライドして制御
...
4. **出力ファイル形状**: 単一ファイル / テーブル毎に複数ファイル / コード生成専用（テンプレート不要）。`GenerateType.resultPathTemplate()`をオーバーライドして制御

4軸とも`GenerateType`enumのオーバーライド可能メソッド1つずつに対応する（`ParameterUnit`のrecord/table/datasetごとのストリーム生成と同じ形）。新しいgenerateTypeを追加する = この4メソッドのうち必要なものをオーバーライドする、が実装の骨格になる。
