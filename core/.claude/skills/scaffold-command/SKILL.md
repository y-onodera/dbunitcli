...
| `command/ScaffoldOption.java` | `execute()` に新規target分岐を追加。`supportsUserTemplate()`（ddl/javaBean）は既存`getStgPath()`/`getTemplatePath()`を流用可。Java側precomputation依存タイプは`writeSchemaTemplate()`同様、専用`{typeName}ScaffoldTemplate.stg/.txt`が必要 |
